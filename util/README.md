...
| `./matchesQuery` | `matchesQuery` | Validate and type-narrow `searchParams` against an allow-list schema |
...
| `./buildQueryString` | `buildQueryString` | Flat params to `?a=1&b=2` (skips `null`/`undefined`) |
| `./createApi` | `createApi` (+ envelope builders) | Throw-free `fetch` factory returning `ApiResponse<T>` (nextjs-conventions §26) |
