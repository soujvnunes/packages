...
const isPropsWithChildren = (props: unknown): props is { children: ReactNode } =>
  props !== null && typeof props === 'object' && 'children' in props

// True when a node is an `<AnimatePresence>` with no rendered children — every child is falsy
// (null/undefined/false/''). Lets a layout skip wrapper markup that would otherwise reserve space
// around an empty presence.
export const isEmptyAnimatePresence = (item: ReactNode) => {
