...
- `soujvnunes/no-needless-use-client` reports the directive when nothing in the file needs the client, and it only reports what it can prove: a hook call, a handler, a browser-only global, a class component, `next/dynamic`, a dot into a named import, an export that is not a component, a value re-export, or a JSX prop that is not provably data each keep the directive.
- `soujvnunes/no-static-jsx-in-client` reports a subtree of three or more elements (the `minElements` option), or a run of static siblings under a dynamic parent, that reads only literals, imports and module constants whose value is itself static, which ships to the browser and hydrates for nothing.
