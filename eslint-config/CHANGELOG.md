...
- 95a7ee6: Add two rules on the Next preset, both at error, for `.jsx` and `.tsx` files that open with `'use client'`. Next's `error` and `global-error` files are skipped, since Next requires them to be client components.
